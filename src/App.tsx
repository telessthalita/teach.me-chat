    ...
    <div className="container">
      <div className="sidebar">
        <details open className="suggestion">
          ...
          {/* Agora, a cada ItemSuggestion damos uma key única */}
          <ItemSuggestion key="html" title="HTML" onClick={() => setTextarea('HTML')} />
          <ItemSuggestion key="css" title="CSS" onClick={() => setTextarea('CSS')} />
          <ItemSuggestion key="javascript" title="JavaScript" onClick={() => setTextarea('JavaScript')} />
          <ItemSuggestion key="typescript" title="TypeScript" onClick={() => setTextarea('TypeScript')} />
        </details>

        <details open className="historic">
          <summary>Histórico</summary>
          {/* Adicionando uma key única para cada item do histórico */}
          {
            getHistoric().map((item, index) => (
              <ItemSuggestion 
                key={index}  // Usando o índice como key (ideal seria um valor único)
                title={item} 
